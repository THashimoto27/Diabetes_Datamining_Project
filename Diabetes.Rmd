...
abstract: "Diabetes is a prevalent chronic disease in the US, affecting millions of
  people and placing a significant financial burden on the economy. Diabetes is a
  prevalent chronic disease in the US, affecting millions of people and placing a
  significant financial burden on the economy. So in this paper, to contribute decreasing
  diabetes, we executed four analysis, (1) building the best predict diabetes model,
  (2) constructing the risk score for diabetes, (3)--(Please insert here)--, and (4)
  ...
  health and BMI. While the variable that does not related to the diabetes or make
  risk score decrease is Heavy alcohol consumption. For the third analysis, we found
  (please insert here) For the fourth analysis, we found the words that are related
  to diabetes academic paper are fasting, metabolic, blood glucose level, cholesterol
  level, stress, inflammatory and pregnancy. We suggested to collect these kind of
...
At first, as we mentioned at Model part, we used apriori function in R with support 0.02 and confidence 0.01 (see [Appendix to look at the support-confidence](## 5-4. Support-Confidence plot)). And with lift >4, we created data to show the  node graph. the following graph is enlarged it around the word "diabetes":
